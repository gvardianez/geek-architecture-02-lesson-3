...
public class UserMapper implements Mapper<User> {
    ...
    public void update(User user) {
        if (checkUpdateUsers(user)) return;
        try {
    ...
    public void update(List<User> users) {
        try {
            for (User user : users) {
                if (checkUpdateUsers(user)) continue;
                updateUser.setString(1, user.getLogin());
                updateUser.setString(2, user.getPassword());
                updateUser.setLong(3, user.getId());
                updateUser.addBatch();
    ...
    private boolean checkUpdateUsers(User user) {
        if (identityMap.get(user.getId()) == null) return false;
        return identityMap.get(user.getId()).equals(user);
    }
